class PurchasesController < ApplicationController
  ...
  def event_purchases
    ...
    if @email_search_string
      @purchases = @event.purchases.where(email: "%#{@email_search_string}%")
    elsif @conf_search_string
      @purchases = @event.purchases.where(conf_num: "%#{@conf_search_string}%")
    elsif @lastfour_search_string
      @purchases = @event.purchases.where(lastfour: "%#{@lastfour_search_string}%")
    else
